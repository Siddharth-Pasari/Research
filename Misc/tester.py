<<<<<<< Updated upstream:Misc/tester.py
import numpy as np
import pandas as pd
...
def update_excel_with_data(data_measurements, file_path):
    ...
    with pd.ExcelWriter(file_path, mode='a', engine='openpyxl', if_sheet_exists='overlay') as writer:
        ...
        # If the file or sheet does not exist yet, it will start from the beginning
        try:
            startrow = writer.sheets['Sheet2'].max_row
        except KeyError:
            startrow = 0
        ...
        # Write the DataFrame to the Excel file
        df.to_excel(writer, sheet_name='Sheet2', startrow=startrow, index=False, header=headers)
...
>>>>>>> Stashed changes:fluoresence.py
root.mainloop()